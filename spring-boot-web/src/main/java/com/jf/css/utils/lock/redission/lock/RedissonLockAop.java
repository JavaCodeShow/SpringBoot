...
public class RedissonLockAop {
	 ...
	 */
	@Pointcut("@annotation(com.jf.css.utils.lock.redission.lock.RedissonLockAnnotation)")
	...
	public String checkLock(ProceedingJoinPoint pjp) throws Throwable {
		...
		// 因为只有一个JSON参数，直接取第一个
		JSONObject param;
		param = (JSONObject) objs[0];
		// 获取该注解的实例对象
		RedissonLockAnnotation annotation = ((MethodSignature) pjp
				.getSignature()).getMethod()
						.getAnnotation(RedissonLockAnnotation.class);
		// 生成分布式锁key的键名，以逗号分隔
		...
		if (StringUtils.isEmpty(lockRedisKey)) {
			...
			if (RedissonLockUtils.tryLock(key, 3000, 5000,
