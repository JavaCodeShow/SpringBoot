...
public class CustomCellWriteHandler extends AbstractColumnWidthStyleStrategy {
	...
	protected void setColumnWidth(WriteSheetHolder writeSheetHolder,
		...
		if (needSetWidth) {
			Map<Integer, Integer> maxColumnWidthMap = CACHE
					.get(writeSheetHolder.getSheetNo());
			...
			if (columnWidth >= 0) {
				...
				if (maxColumnWidth == null || columnWidth > maxColumnWidth) {
					...
					writeSheetHolder.getSheet().setColumnWidth(
							cell.getColumnIndex(), columnWidth * 256);
				}
	...
	private Integer dataLength(List<CellData> cellDataList, Cell cell,
			Boolean isHead) {
		if (isHead) {
