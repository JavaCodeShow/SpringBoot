...
public class ConcurrentProtectedCacheManager {
        ...
        if (expired) {
            ...
            if (lock) {
                try {
                    ...
                    distributeLockService.unlock(concurrentCacheKey);
                }
            }
        }
    ...
    private <T> T cacheDataFromSupplier(String cacheKey, int cacheLogicalExpireSeconds, int cacheDataKeyExpireSeconds, Supplier<T> supplier) {
        ...
        globalCacheService.hMSet(cacheKey, resultMap);
        globalCacheService.setExpire(cacheKey, (long) cacheDataKeyExpireSeconds);
        return t;
    }
